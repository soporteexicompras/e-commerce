...
<html lang="{{ $locale }}" dir="{{ $localeDir }}">
	<head>
		...
		<title>Exicompras — Panel de administración</title>
		@php
		...
		<style nonce="{{ app( 'aimeos.context' )->get( false )->nonce() }}">
			...
			.app-menu .icon {cursor: pointer; padding: 0.75rem; height: 2.5rem; width: 2.5rem; margin: 0 0.25rem; color: var(--bs-menu, #f0f2ff) !important;}
			.app-menu button {border: none; color: var(--bs-menu, #f0f2ff) !important; background-color: transparent; padding: 0;}
			.app-menu button:hover { opacity: .75; }
			#logout-form {display: inline-block}
			.aimeos .logo { display:flex; align-items:center; justify-content:center; font-size:1.2rem; font-weight:700; color:#fff; text-decoration:none; }
			.aimeos .logo:hover { color:#fff; text-decoration:none; }
			body {
				@foreach( app( 'aimeos.context' )->get( false )->locale()->getSiteItem()->getConfigValue( 'theme/default', [] ) as $key => $value )
					{{ $key }}: {{ $value }};
				@endforeach
			}
		</style>
