	...
	<div class="row item-container">
		...
		<div class="col-xl-9 item-content tab-content">
			...
			<div id="basic" class="item-basic tab-pane fade show active" role="tabpanel" aria-labelledby="basic">
				...
				<?php
					$themeDefaults = $this->get( 'themeData/default', [] );
					$logoHeight    = max( 32, min( 200, (int) ( $themeDefaults['--ai-nav-logo-height'] ?? 64 ) ) );
				?>

				<div class="row item-media">
					<div class="col-xl-6 item-logo">
						<div class="box">
							<p style="font-weight:600;margin-bottom:.5rem"><?= $enc->html( $this->translate( 'admin', 'Shop logo' ) ) ?> <small class="text-muted">(navbar)</small></p>
							...
							<div class="form-group media-preview">
								...
								<?php if( $logos = $this->get( 'itemData/locale.site.logo' ) ) : ?>
									<img id="exicompras-logo-preview" class="item-preview"
										src="<?= $enc->attr( $this->content( end( $logos ) ) ) ?>"
										style="height:<?= $enc->attr( $logoHeight ) ?>px;width:auto;max-width:100%;transition:height .15s">
								<?php else : ?>
							...
							<div class="form-group row mt-3">
								<label class="col-sm-12 form-control-label mb-1" style="font-weight:600">
									Tamaño del logo <span id="exicompras-logo-size-label" style="color:var(--bs-primary)"><?= $enc->html( $logoHeight ) ?>px</span>
								</label>
								<div class="col-sm-12">
									<div class="d-flex align-items-center gap-2">
										<span style="font-size:.75rem;opacity:.55">32px</span>
										<input
											id="exicompras-logo-slider"
											type="range"
											class="form-range flex-grow-1"
											min="32" max="200" step="4"
											value="<?= $enc->attr( $logoHeight ) ?>"
										>
										<span style="font-size:.75rem;opacity:.55">200px</span>
									</div>
									<small class="text-muted">Arrastra para ajustar — la vista previa se actualiza en tiempo real</small>
								</div>
							</div>
					...
					<div class="col-xl-6 item-icon">
						<div class="box">
							<p style="font-weight:600;margin-bottom:.5rem"><?= $enc->html( $this->translate( 'admin', 'Shop icon' ) ) ?> <small class="text-muted">(favicon / pestaña)</small></p>
...
<script>
(function() {
	var slider  = document.getElementById('exicompras-logo-slider');
	var label   = document.getElementById('exicompras-logo-size-label');
	var preview = document.getElementById('exicompras-logo-preview');
	if (!slider) return;

	/* Busca el input real del panel Tema que tiene el name correcto */
	function getRealInput() {
		return document.querySelector('input[name$="[--ai-nav-logo-height]"]');
	}
	...
	function update(val) {
		...
		/* Sincroniza el input real del panel Tema */
		var real = getRealInput();
		if (real && real !== slider) real.value = val;
	}

	slider.addEventListener('input', function() { update(this.value); });

	/* Al cargar la página sincroniza ambos */
	update(slider.value);

	/* Seguridad extra: sincroniza justo antes de enviar */
	var form = slider.closest('form');
	if (form) {
		form.addEventListener('submit', function() {
			var real = getRealInput();
			if (real && real !== slider) real.value = slider.value;
		});
	}
})();
</script>
